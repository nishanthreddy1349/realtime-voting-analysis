...
BASE_URL = 'https://randomuser.me/api/?nat=us'   #https://randomuser.me/api/?nat=gb
PARTIES = ["Democratic Party", "Republican Party", "Third Parties"]
random.seed(42)
...
def generate_biography(party):
    if party == "Democratic Party":
        return "A Democratic candidate with a focus on social justice and economic equality."
    elif party == "Republican Party":
        return "A Republican candidate advocating for limited government and free-market policies."
    else:
        return "An independent candidate bringing fresh ideas to the political landscape."

def generate_campaign_platform(party):
    if party == "Democratic Party":
        return "Strengthening healthcare, investing in education, and combating climate change."
    elif party == "Republican Party":
        return "Lowering taxes, reducing regulations, and strengthening national security."
    else:
        return "Promoting political reform, fiscal responsibility, and bipartisanship."
...
def generate_candidate_data(candidate_number, total_parties):
    ...
    if response.status_code == 200:
        ...
        return {
            ...
            "biography": generate_biography(PARTIES[candidate_number % total_parties]),
            "campaign_platform": generate_campaign_platform(PARTIES[candidate_number % total_parties]),
...
def insert_voters(conn, cur, voter):
    try:
        ...
        # Ignore the duplicate record and continue with the next one
        conn.rollback()
        print(f"Duplicate voter_id: {voter['voter_id']}. Skipping the record.")
